...
\begin{quote}
\textbf{Important Hardware Split:} Phase A tests (static
characterisation, steady-state \((T, \omega)\) grids) require constant
torque at constant speed, which mathematically cannot be achieved on a
swinging pendulum. \textbf{Phase A must be performed on a standard
active dynamometer} (motor coupled to a load motor). \textbf{Phase B
(dynamic policy replay)} requires realistic transient gravitational
loading, which is provided by the pendulum arm fixture detailed below.
\end{quote}

\subsubsection{3.1 Concept}\label{concept}

For Phase B policy testing, rather than machining steel flywheel discs,
the motor is clamped securely to the edge of a sturdy workbench with its
shaft horizontal. A steel or aluminium flat bar is attached to the
output shaft via a rigid jaw coupler, pointing downwards under gravity,
free to swing in the vertical plane.
...
All tests evaluate the \textbf{96V GaN inverter board} driving the
AKE80-8 motor. To demonstrate the benefits of the GaN architecture
without needing a separate Si board, we define a \textbf{``Si-equivalent
baseline''} (48V bus, 20--50 kHz \(f_{sw}\), 500 ns dead time) and
compare it against the \textbf{``GaN-advanced''} configuration (96V bus,
up to 100 kHz \(f_{sw}\), \textless100 ns dead time).

\textbf{Test order:} Phase 0 (Sim) → Phase A (T1--T4) → Phase B
(T5--T8). Allow the motor to cool to ambient between physical tests.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsubsection{Phase 0 --- MATLAB/Simulink Co-Simulation (Virtual
Dyno)}\label{phase-0-matlabsimulink-co-simulation-virtual-dyno}

\textbf{Purpose:} Establish a theoretical ``ground truth'' performance
baseline derived directly from MOSFET and motor datasheet parameters,
before any physical hardware is spun.

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{Virtual Dyno Simulation:} Model the GaN inverter, the AKE80-8
  motor, and a standard load motor in Simscape Electrical. Run nested
  loops commanding constant speeds and constant load torques to generate
  a theoretical \((T, \omega)\) efficiency heatmap.
\item
  \textbf{Loss Breakdown:} Extract theoretical switching vs.~conduction
  loss ratios for both the ``Si-baseline'' and ``GaN-advanced''
  parameters.
\item
  \textbf{FOC Pre-Tuning:} Safely tune the PI gains (\(K_p, K_i\)) for
  the 96V system in simulation to ensure stable tracking bandwidth
  before running high-power physical tests.
\end{enumerate}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsubsection{Phase A --- Hardware Characterisation (Standard
Dyno)}\label{phase-a-hardware-characterisation-standard-dyno}

\begin{quote}
\textbf{Prerequisites:} Phase 0 theoretical baselines established. Motor
mounted to the \textbf{Standard Active Dynamometer} and coupled to the
load motor.
\end{quote}
...
Setup & \textbf{Standard Active Dynamometer} (motor coupled to load
motor) \\
Torques & 1, 3, 6, 9, 12 Nm (held constant by load motor) \\
Speeds & 10, 30, 60, 100, 150 rpm (held constant by drive motor) \\
...
\bottomrule\noalign{}
\endlastfoot
Profile & Constant speed \(\omega\), varying load torque \\
\(f_{sw}\) values & 20, 50, 75, 100 kHz \\
Duration & 60 s per frequency \\
Setup & Standard Active Dynamometer, fixed 96V bus, same dead time \\
\end{longtable}
}
...
\bottomrule\noalign{}
\endlastfoot
Profile & Constant speed, constant load torque (e.g.~100 RPM, 6 Nm) \\
Dead times & 500 ns (Si-equivalent), 200 ns, 100 ns, 50 ns
(GaN-optimised) \\
Setup & Standard Active Dynamometer, 50 kHz \(f_{sw}\), 96V bus \\
\end{longtable}
}
...
(GaN-advanced) \\
Profile & Continuous steady-state operation at the worst-case
\((T,\omega)\) from T1 \\
Setup & Standard Active Dynamometer \\
...
on Phase A results. \textbf{Motor moved to Pendulum Arm Fixture.} Policy
trajectory files exported (see §4). Pendulum arm inertia verified.
\end{quote}
