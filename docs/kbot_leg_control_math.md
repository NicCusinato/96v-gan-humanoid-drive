...
- $J_c(q)^T\lambda$ is the contribution of foot contact forces.

This equation is valid for both stance and swing; in stance, the contact forces $\lambda$ enforce non-penetration and friction constraints at the feet.
...
- $J_{c,j}(q)$ is the contact Jacobian projected onto the actuated joints.
...
If both feet are firmly in contact and $\lambda$ is chosen such that the net wrench balances the robot's weight and external loads, $\tau_{\text{stand}}$ is the joint torque required to hold the given posture. In practice, we obtain this quantity from inverse dynamics or bias-force computation.
...
- Inverse dynamics via `mj_inverse`, which computes generalized forces given $(q,\dot{q},\ddot{q})$.
...
This provides a model-consistent estimate of the torque needed to counter gravity and static loads for a given configuration.
...
- $\tau_g(q)$ is the gravity-compensation torque from the model.
...
with tracking error $e_i = q_{d,i} - q_{j,i}$ and effective inertia $m_{j,i}$ at joint $i$. This facilitates gain tuning in terms of desired damping ratio and natural frequency.
...
1. Rigid-body links and ideal joints; link flexibilities and backlash are neglected.
2. Any modeled friction is included in $h_j(\cdot)$ and thus partially compensated by $\tau_g$.
3. During standing and squat tests, both feet remain in full, non-slipping contact with the ground, and the contact solver enforces the associated constraints.
